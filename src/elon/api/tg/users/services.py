...
def one_user_by_chat_id(request, chat_id=None):
    try:
        ...
        print("Error during fetching user by chat_id in services.py:", e)


def one_telegram_user_by_chat_id(request, chat_id=None):
    try:
        with closing(connection.cursor()) as cursor:
            cursor.execute("""SELECT * FROM users_telegramuser WHERE chat_id = %s""", [chat_id])
            data = dictfetchone(cursor)
            if data:
                result = OrderedDict([
        ...
        print("Error during fetching telegram user by chat_id in services.py:", e)
